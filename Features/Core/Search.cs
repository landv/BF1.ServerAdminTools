...
public static class Search
{
    ...
    public static Task<string> SearchMemory(string mask)
    {
        ...
        {
            // 扫描内存信息
            if (VirtualQueryEx(Memory.GetProcessHandle(), new IntPtr(baseAddress), out mbi, size) == 0)
                break;
    ...
    private static long FindPattern(string pattern, long baseAddress, int moduleSize)
    {
        ...
        WinAPI.ReadProcessMemory(Memory.GetProcessHandle(), baseAddress, localModulebytes, moduleSize, out _);
