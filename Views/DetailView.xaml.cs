...
public partial class DetailView : UserControl
{
    ...
    private async void GetFullServerDetails(bool isInit = false)
    {
        if (!string.IsNullOrEmpty(Globals.SessionId))
        {
            if (!string.IsNullOrEmpty(Globals.GameId))
            {
                ...
                if (!isInit)
                    NotifierHelper.Show(NotifierType.Information, $"正在获取服务器 {Globals.GameId} 详细数据中...");
                ...
                if (result.IsSuccess)
                {
                    ...
                    if (!isInit)
                        NotifierHelper.Show(NotifierType.Success, $"获取服务器 {Globals.GameId} 详细数据成功  |  耗时: {result.ExecTime:0.00} 秒");
                }
                else
                {
                    if (!isInit)
                        NotifierHelper.Show(NotifierType.Error, $"获取服务器 {Globals.GameId} 详细数据失败 {result.Message}  |  耗时: {result.ExecTime:0.00} 秒");
                }
            }
            else
            {
    ...
    private async void Button_LeaveCurrentGame_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(Globals.SessionId))
        {
            if (!string.IsNullOrEmpty(Globals.GameId))
            {
                ...
                if (result.IsSuccess)
                {
                    NotifierHelper.Show(NotifierType.Success, $"离开服务器 {Globals.GameId} 成功  |  耗时: {result.ExecTime:0.00} 秒");
                }
                else
                {
                    NotifierHelper.Show(NotifierType.Error, $"离开服务器 {Globals.GameId} 失败 {result.Message}  |  耗时: {result.ExecTime:0.00} 秒");
                }
            }
            else
            {
    ...
    private async void ListBox_Map_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        if (index != -1)
        {
            ...
            if (!string.IsNullOrEmpty(Globals.PersistedGameId))
            {
                ...
                {
                    NotifierHelper.Show(NotifierType.Information, $"正在更换服务器 {Globals.GameId} 地图为 {currMap.mapPrettyName} 中...");

                    var result = await BF1API.ChangeServerMap(Globals.PersistedGameId, index);
                    if (result.IsSuccess)
                    {
                        NotifierHelper.Show(NotifierType.Success, $"更换服务器 {Globals.GameId} 地图为 {currMap.mapPrettyName} 成功  |  耗时: {result.ExecTime:0.00} 秒");
                    }
                    else
                    {
                        NotifierHelper.Show(NotifierType.Error, $"更换服务器 {Globals.GameId} 地图为 {currMap.mapPrettyName} 失败 {result.Message}  |  耗时: {result.ExecTime:0.00} 秒");
                    }
                }
            }
            else
            {
                NotifierHelper.Show(NotifierType.Warning, "PersistedGameId异常，请重新获取服务器详细信息");
            }
        }
    ...
    private async void Button_RemoveSelectedAdmin_Click(object sender, RoutedEventArgs e)
    {
        ...
        NotifierHelper.Show(NotifierType.Information, $"正在移除服务器管理员 {rSPInfo.displayName} 中...");

        var result = await BF1API.RemoveServerAdmin(rSPInfo.personaId);
        if (result.IsSuccess)
        {
            NotifierHelper.Show(NotifierType.Success, $"移除服务器管理员 {rSPInfo.displayName} 成功  |  耗时: {result.ExecTime:0.00} 秒");
        }
        else
        {
            NotifierHelper.Show(NotifierType.Error, $"移除服务器管理员 {rSPInfo.displayName} 失败 {result.Message}  |  耗时: {result.ExecTime:0.00} 秒");
        }
    }
    ...
    private async void Button_AddNewAdmin_Click(object sender, RoutedEventArgs e)
    {
        ...
        NotifierHelper.Show(NotifierType.Information, $"正在增加服务器管理员 {TextBox_NewAdminName.Text} 中...");

        var result = await BF1API.AddServerAdmin(TextBox_NewAdminName.Text);
        if (result.IsSuccess)
        {
            NotifierHelper.Show(NotifierType.Success, $"增加服务器管理员 {TextBox_NewAdminName.Text} 成功  |  耗时: {result.ExecTime:0.00} 秒");
        }
        else
        {
            NotifierHelper.Show(NotifierType.Error, $"增加服务器管理员 {TextBox_NewAdminName.Text} 失败 {result.Message}  |  耗时: {result.ExecTime:0.00} 秒");
        }
    }
    ...
    private async void Button_RemoveSelectedVIP_Click(object sender, RoutedEventArgs e)
    {
        ...
        NotifierHelper.Show(NotifierType.Information, $"正在移除服务器VIP {rSPInfo.displayName} 中...");

        var result = await BF1API.RemoveServerVip(rSPInfo.personaId);
        if (result.IsSuccess)
        {
            NotifierHelper.Show(NotifierType.Success, $"移除服务器VIP {rSPInfo.displayName} 成功  |  耗时: {result.ExecTime:0.00} 秒");
        }
        else
        {
            NotifierHelper.Show(NotifierType.Error, $"移除服务器VIP {rSPInfo.displayName} 失败 {result.Message}  |  耗时: {result.ExecTime:0.00} 秒");
        }
    }
    ...
    private async void Button_AddNewVIP_Click(object sender, RoutedEventArgs e)
    {
        ...
        NotifierHelper.Show(NotifierType.Information, $"正在增加服务器VIP {TextBox_NewVIPName.Text} 中...");

        var result = await BF1API.AddServerVip(TextBox_NewVIPName.Text);
        if (result.IsSuccess)
        {
            NotifierHelper.Show(NotifierType.Success, $"增加服务器VIP {TextBox_NewVIPName.Text} 成功  |  耗时: {result.ExecTime:0.00} 秒");
        }
        else
        {
            NotifierHelper.Show(NotifierType.Error, $"增加服务器VIP {TextBox_NewVIPName.Text} 失败 {result.Message}  |  耗时: {result.ExecTime:0.00} 秒");
        }
    }
    ...
    private async void Button_RemoveSelectedBAN_Click(object sender, RoutedEventArgs e)
    {
        ...
        NotifierHelper.Show(NotifierType.Information, $"正在移除服务器BAN {rSPInfo.displayName} 中...");

        var result = await BF1API.RemoveServerBan(rSPInfo.personaId);
        if (result.IsSuccess)
        {
            NotifierHelper.Show(NotifierType.Success, $"移除服务器BAN {rSPInfo.displayName} 成功  |  耗时: {result.ExecTime:0.00} 秒");
        }
        else
        {
            NotifierHelper.Show(NotifierType.Error, $"移除服务器BAN {rSPInfo.displayName} 失败 {result.Message}  |  耗时: {result.ExecTime:0.00} 秒");
        }
    }
    ...
    private async void Button_AddNewBAN_Click(object sender, RoutedEventArgs e)
    {
        ...
        NotifierHelper.Show(NotifierType.Information, $"正在增加服务器BAN {TextBox_NewBANName.Text} 中...");

        var result = await BF1API.AddServerBan(TextBox_NewBANName.Text);
        if (result.IsSuccess)
        {
            NotifierHelper.Show(NotifierType.Success, $"增加服务器BAN {TextBox_NewBANName.Text} 成功  |  耗时: {result.ExecTime:0.00} 秒");
        }
        else
        {
            NotifierHelper.Show(NotifierType.Error, $"增加服务器BAN {TextBox_NewBANName.Text} 失败 {result.Message}  |  耗时: {result.ExecTime:0.00} 秒");
        }
    }
    ...
    private async void Button_KickSelectedSpectator_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(Globals.SessionId))
        {
            ...
            NotifierHelper.Show(NotifierType.Information, $"正在踢出玩家 {info.Name} 中...");
            ...
            if (result.IsSuccess)
            {
                NotifierHelper.Show(NotifierType.Success, $"踢出玩家 {info.Name} 成功  |  耗时: {result.ExecTime:0.00} 秒");
            }
            else
            {
                NotifierHelper.Show(NotifierType.Error, $"踢出玩家 {info.Name} 失败 {result.Message}  |  耗时: {result.ExecTime:0.00} 秒");
            }
        }
        else
        {
        ...
        if (!string.IsNullOrEmpty(Globals.SessionId))
        {
            if (!string.IsNullOrEmpty(Globals.ServerId))
            {
                ...
                if (result.IsSuccess)
                {
                    ...
                    NotifierHelper.Show(NotifierType.Success, $"获取服务器 {Globals.ServerId} 数据成功  |  耗时: {result.ExecTime:0.00} 秒");
                }
                else
                {
                    NotifierHelper.Show(NotifierType.Error, $"获取服务器 {Globals.ServerId} 数据失败 {result.Message}  |  耗时: {result.ExecTime:0.00} 秒");
                }
            }
            else
            {
                NotifierHelper.Show(NotifierType.Warning, "请先进入服务器获取ServerID");
            }
        }
        else
        {
    ...
    private async void Button_UpdateServer_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(Globals.SessionId))
        {
            if (!string.IsNullOrEmpty(Globals.ServerId))
            {
                ...
                if (result.IsSuccess)
                {
                    NotifierHelper.Show(NotifierType.Success, $"更新服务器 {Globals.ServerId} 数据成功  |  耗时: {result.ExecTime:0.00} 秒");
                }
                else
                {
                    NotifierHelper.Show(NotifierType.Error, $"更新服务器 {Globals.ServerId} 数据失败 {result.Message}  |  耗时: {result.ExecTime:0.00} 秒");
                }
            }
            else
            {
